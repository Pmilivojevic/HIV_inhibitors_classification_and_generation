    ...
    def validation(
        ...
        with torch.no_grad():
            ...
            if epoch_loss < best_val_loss:
                ...
                metrics['mcc'] = matthews_corrcoef(val_labels, val_preds)
                metrics['f2'] = fbeta_score(val_labels, val_preds, beta=2)
                precision, recall, _ = precision_recall_curve(val_labels, val_preds_float)
    ...
    def train_compose(self):
        if (not os.listdir(
                self.config.models
                    ...
                    os.path.join(self.config.models, 'best_retrain')
                    ) and not os.path.exists(
                            os.path.join(self.config.stats, 'best_retrain')
                        )):
                    ...
                    for epoch in tqdm(range(params.num_epochs)):
                        if early_stopping_counter <= params.early_stopping:
                            ...
                            if float(val_epoch_loss) < best_val_loss:
                                torch.save(
                                    model.state_dict(),
                                    os.path.join(models_path, f'model_{epoch}.pth')
                                )
                            ...
                            for type, train_metric, val_metric in zip(
                                                                    plot_metrics,
                                                                    train_metrics,
                                                                    val_metrics
                                                                ):
            ...
            if self.config.tuning:
                ...
                for folder in os.listdir(self.config.stats):
                    ...
                    if self.config.params['BEST_PARAMETERS'] == tuning_params:
                        ...
                        try:
                            best_stats_folder_path.rename(
                                self.config.stats.joinpath('best_params')
                            )
                            print(
                                f"""Folder renamed from {best_stats_folder_path} to 
                                {self.config.stats.joinpath('best_params')}"""
                            )
                        ...
                        try:
                            best_model_folder_path.rename(
                                self.config.models.joinpath('best_params')
                            )
                            print(
                                f"""Folder renamed from {best_stats_folder_path} to 
                                {self.config.stats.joinpath('best_params')}"""
